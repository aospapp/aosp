...
final class AudioVolumeGroupCallbackHelper extends AudioManager.VolumeGroupCallback {
    ...
    public boolean waitForExpectedVolumeGroupChanged(long timeOutMs) {
        ...
        while (elapsedTimeMs < ASYNC_TIMEOUT_MS && mVolumeGroupChanged.getCount() > 0) {
            try {
                mVolumeGroupChanged.await(ASYNC_TIMEOUT_MS, TimeUnit.MILLISECONDS);
            } catch (InterruptedException e) {
                Log.w(TAG, "wait interrupted");
            }
            elapsedTimeMs = System.currentTimeMillis() - startTimeMs;
        }
        return mVolumeGroupChanged.getCount() <= 0;
    }
}
